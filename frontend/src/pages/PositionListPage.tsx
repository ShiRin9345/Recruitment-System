...
export const PositionListPage = () => {
  ...
  const canEdit = (position: Position) => role === "ADMIN" && ["DRAFT", "REJECTED"].includes(position.status);
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <h2 className="text-2xl font-semibold tracking-normal">岗位管理</h2>
          <p className="mt-1 text-sm text-slate-500">管理岗位草稿、审批、发布和关闭流程</p>
        </div>
        {role === "ADMIN" ? (
          <Button asChild>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>支持按岗位名称模糊搜索和状态筛选</CardDescription>
        </CardHeader>
        <CardContent>
